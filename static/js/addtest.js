...
window.onsubmit = () => {
	...
	for (let i = 0; i < questions.length; i++) {
		...
		test.push({ question, answerType, answerVariants });
	...
	document.querySelector(`#test`).innerHTML = JSON.stringify(test);
	document.querySelector(`#answers`).innerHTML = JSON.stringify(answers);
	return true;
};
